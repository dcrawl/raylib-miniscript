  ...
  case "$name" in
    ...
    vp8_audio_diag_scaffold_smoke)
      if /usr/bin/grep -q "audio diag: packets=" "$log" \
        ...
        echo "[PASS] $name"
        /usr/bin/grep -n "audio diag: packets=\|audio scaffold: supported=\|audio decode stub:\|audio decode state:\|audio decode batch: count=\|audio decode reset:\|Audio diag/scaffold smoke done" "$log" | /usr/bin/tail -n 10
        PASS_COUNT=$((PASS_COUNT + 1))
      else
        echo "[FAIL] $name (expected audio diagnostics/scaffold markers missing)"
        /usr/bin/grep -n "audio diag: packets=\|audio scaffold: supported=\|audio decode stub:\|audio decode state:\|audio decode batch: count=\|audio decode reset:\|Audio diag/scaffold smoke failed:\|Audio diag/scaffold smoke done" "$log" || true
        FAIL_COUNT=$((FAIL_COUNT + 1))
      fi
      ;;
    *)
